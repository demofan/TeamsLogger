...
namespace TeamsLogger.Core
{
    public class Logger : ILogger
    {
        ...
        public void PostRunningLog(string colorHexCode = null)
        {
            ...
            {
                if (_hasException)
                    _card.ThemeColor = DefaultColor.ErrorColor;
                else if (_hasWarning && !_hasException)
                    _card.ThemeColor = DefaultColor.WarningColor;
                else
                    _card.ThemeColor = DefaultColor.SuccessColor;
            }
            else
            {
            ...
            if (string.IsNullOrEmpty(color) && _loggerConfiguration.AutomaticallySetColor)
            {
                switch (severity)
                {
                    case LogSeverity.Info:
                        color = DefaultColor.InfoColor;
                        break;
                    case LogSeverity.Warn:
                        color = DefaultColor.WarningColor;
                        break;
                    case LogSeverity.Error:
                        color = DefaultColor.ErrorColor;
                        break;
                    default:
        ...
        private string GetColorCode(LogSeverity severity)
        {
            ...
            {
                case LogSeverity.Info:
                    return DefaultColor.InfoColor;
                case LogSeverity.Warn:
                    return DefaultColor.WarningColor;
                case LogSeverity.Error:
                    return DefaultColor.ErrorColor;
                default:
                    throw new ArgumentOutOfRangeException(nameof(severity), severity, null);
            }
        }
